...
export async function batchQueryVulnerabilities(
  ...
  if (!res.ok) return queries.map(() => ({ vulns: [] }));
  const data = await res.json();
  return data.results || [];
}

export async function getVulnDetail(vulnId: string): Promise<any> {
  const res = await fetchWithTimeout(`${OSV_API}/vulns/${encodeURIComponent(vulnId)}`);
  if (!res.ok) throw new Error(`OSV.dev returned ${res.status} for ${vulnId}`);
  return res.json();
}
