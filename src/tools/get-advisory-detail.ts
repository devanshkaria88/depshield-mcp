...
export async function getAdvisoryDetail(params: { vulnId: string }): Promise<string> {
  let vuln: any;
  try {
  ...
  if (vuln.summary) report += `📝 Summary: ${vuln.summary}\n\n`;
  report += `⚡ Severity: ${sev.label}${sev.score !== null ? ` (CVSS: ${sev.score})` : ""}\n`;

  if (vuln.published) report += `📅 Published: ${vuln.published}\n`;
  if (vuln.modified) report += `📅 Modified: ${vuln.modified}\n`;
  report += `\n`;
      ...
      if (aff.ranges) {
        for (const range of aff.ranges) {
          if (range.events) {
            ...
            if (introduced) report += `     Introduced: ${introduced}\n`;
            if (fixed) report += `     Fixed: ${fixed}\n`;
          }
        }
      }
