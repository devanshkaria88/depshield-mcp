...
import { parseSeverity, parseFixedVersion, formatVulnList, type ParsedVuln } from "../utils.js";
...
export async function checkDependency(params: {
  ...
  const parsed: ParsedVuln[] = vulns.map((v) => {
  ...
  const fixedVersions = parsed.map((p) => p.fixedIn).filter(Boolean) as string[];
