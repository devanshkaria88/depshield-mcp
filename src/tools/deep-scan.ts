...
import { parseSeverity, parseFixedVersion, stripVersionRange, sortVersionsDescending, type ParsedVuln } from "../utils.js";
...
interface WeirdPattern {
  type: "NEW_DEP" | "LOW_DOWNLOADS" | "NONEXISTENT";
  dep: string;
  detail: string;
}

async function resolveDepVersion(name: string): Promise<{ version: string; exists: boolean }> {
  const result = await checkPackageExists(name);
  if (!result.exists) return { version: "unknown", exists: false };
  return { version: result.latestVersion!, exists: true };
}

function getDepsForVersion(metadata: any, version: string): Record<string, string> {
  return metadata.versions?.[version]?.dependencies || {};
}

function getPreviousVersion(metadata: any, currentVersion: string): string | null {
...
export async function deepScan(params: {
  ...
  if (!version) return `❌ Could not resolve version for '${params.name}'.`;
  ...
  const resolvedDirect: ResolvedDep[] = [];
  const resolveResults = await Promise.all(
    directDepNames.map(async (depName) => {
  ...
  let resolvedTransitive: ResolvedDep[] = [];
  if (maxDepth >= 2) {
    const subDepFetches = resolvedDirect.map(async (dep) => {
      try {
        ...
        const subResults: ResolvedDep[] = [];
        for (const [subName, subRange] of Object.entries(subDeps)) {
          const stripped = stripVersionRange(subRange as string);
          if (stripped) {
            subResults.push({
              name: subName,
              version: stripped,
  ...
  const results: DepScanResult[] = uniqueDeps.map((dep, i) => {
    const vulns = (osvResults[i]?.vulns || []).map((v: any) => {
  ...
  if (maxDepth >= 2) report += `   Sub-deps: ${resolvedTransitive.length} (${uniqueDeps.length - resolvedDirect.length} unique)\n`;
