...
export async function suggestAlternative(params) {
    ...
    const filtered = results.filter((r) => r.name !== params.name);
    ...
    for (let i = 0; i < top.length; i++) {
        ...
        if (pkg.date)
            report += `   📅 Last published: ${new Date(pkg.date).toLocaleDateString()}\n`;
        report += `\n`;
    }
    report += `💡 Run check_dependency on any of these before installing.`;
    return report;
}
//# sourceMappingURL=suggest-alternative.js.map