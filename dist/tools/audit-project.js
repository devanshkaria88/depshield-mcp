...
import { parseSeverity, parseFixedVersion, stripVersionRange } from "../utils.js";
function parsePackageJson(content, includeDevDeps) {
    const pkg = JSON.parse(content);
    const deps = [];
    if (pkg.dependencies) {
        for (const [name, ver] of Object.entries(pkg.dependencies)) {
            const version = stripVersionRange(ver);
            if (version)
...
function parseRequirementsTxt(content) {
    return content
        ...
        if (match)
            return { name: match[1], version: match[2], isDev: false };
        const nameOnly = line.match(/^([a-zA-Z0-9_.-]+)/);
        if (nameOnly)
            return { name: nameOnly[1], version: "latest", isDev: false };
        return null;
    })
        .filter(Boolean);
}
