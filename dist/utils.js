...
export function parseFixedVersion(vuln) {
...
export function formatVulnList(vulns) {
    return vulns
        .map((v) => `• ${v.id} (${v.severity}): ${v.summary}${v.fixedIn ? ` — Fixed in ${v.fixedIn}` : ""}`)
        .join("\n");
}
/**
 ...
 */
export function stripVersionRange(version) {
